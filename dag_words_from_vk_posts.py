...
with DAG(dag_id='dag_words_from_vk_posts', default_args=args, schedule_interval=None) as dag:
    create_vk_post_words_dataset = PythonOperator(
        task_id='download_words',
        python_callable=download_words,
        dag=dag
    )
    insert_in_words_table = PythonOperator(
        task_id='put_in_db',
        python_callable=put_in_db,
        dag=dag
    )
    create_vk_post_words_dataset >> insert_in_words_table